...
const AssetsTab = ({ ppm_id, refetch }) => {
  ...
  const { data: availableAssetsData, loading: availableAssetsLoading, error: availableAssetsError, refetch: refetchAvailableAssets } = useQuery(GET_ASSETS_BY_DISCIPLINE_AND_BUILDING, {
    skip: !disc_id || !bld_id,
  ...
  useEffect(() => {
    if (linkedAssetsData && availableAssetsData) {
      const linked = linkedAssetsData.ppm_asset_service_plan
        .filter(l => l.asset.location?.fk_bld_id === parseInt(bld_id))
        .map(a => a.asset);
  ...
  if (linkedAssetsLoading || availableAssetsLoading) return <p>Loading...</p>;
  if (linkedAssetsError) return <p>Error loading linked assets: {linkedAssetsError.message}</p>;
  if (availableAssetsError) return <p>Error loading available assets: {availableAssetsError.message}</p>;
